...
%   X = U*S*V'. It is a faster version of [U,S,V]=svds(X,rank(X))
%   size£ºU:m*k, S:k*k, V:n*k   k=rank(X)
%
...
%   It is a faster version of [U,S,V]=svds(X,k)
%   size£ºU:m*k, S:k*k, V:n*k   k=ReducedDim, when ReducedDim<=rank(X)
%   size£ºU:m*k, S:k*k, V:n*k   k=rank(X), when ReducedDim>rank(X)
%
